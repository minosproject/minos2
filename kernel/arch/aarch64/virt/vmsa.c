...
static void vmsa_state_init(struct vcpu *vcpu, void *context)
{
	...
	c->vttbr_el2 = generate_vttbr_el2(vm->vmid, vtop(vm->vs.pgdp));
	c->ttbr0_el1 = 0;
	c->ttbr1_el1 = 0;
	c->mair_el1 = 0;
	c->tcr_el1 = 0;
...
static void vmsa_state_save(struct vcpu *vcpu, void *context)
{
	...
	c->vtcr_el2 = read_sysreg(ARM64_VTCR_EL2);
	c->vttbr_el2 = read_sysreg(ARM64_VTTBR_EL2);
	c->ttbr0_el1 = read_sysreg(ARM64_TTBR0_EL1);
	c->ttbr1_el1 = read_sysreg(ARM64_TTBR1_EL1);
	c->mair_el1 = read_sysreg(ARM64_MAIR_EL1);
	c->tcr_el1 = read_sysreg(ARM64_TCR_EL1);
	c->par_el1 = read_sysreg(ARM64_PAR_EL1);
	c->amair_el1 = read_sysreg(ARM64_AMAIR_EL1);
	dsb();
}
...
static void vmsa_state_restore(struct vcpu *vcpu, void *context)
{
	...
	write_sysreg(c->vtcr_el2, ARM64_VTCR_EL2);
	write_sysreg(c->vttbr_el2, ARM64_VTTBR_EL2);
	write_sysreg(c->ttbr0_el1, ARM64_TTBR0_EL1);
	write_sysreg(c->ttbr1_el1, ARM64_TTBR1_EL1);
	write_sysreg(c->mair_el1, ARM64_MAIR_EL1);
	write_sysreg(c->amair_el1, ARM64_AMAIR_EL1);
	write_sysreg(c->tcr_el1, ARM64_TCR_EL1);
	write_sysreg(c->par_el1, ARM64_PAR_EL1);
