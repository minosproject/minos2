...
void task_sleep(uint32_t delay)
{
	...
	/*
	 * task sleep will wait for the sleep time expired
...
void switch_to_task(struct task *cur, struct task *next)
{
	...
	if (!task_is_running(cur) && (cur->delay >0))
...
void irq_return_handler(void)
{
	 ...
	 */
	if ((ti->preempt_count > 0) || !(ti->flags & __TIF_NEED_RESCHED) ||
