...
int host_vdev_init(struct vm *vm, struct vdev *vdev, unsigned long base, uint32_t size)
{
	...
	if (base) {
		va = find_vspace_area(&vm->vs, base);
		if (!va)
			va = split_vspace_area(&vm->vs, base, size, VM_GUEST_VDEV);
	} else {
		va = alloc_vspace_area_page(&vm->vs, size, VM_GUEST_VDEV);
		base = va->start;
	}
